...
// scripting function
export default function snap(site: string) {
  if (site === null) return

  const sendMessage = (message: JobPostingMessage) => {
    chrome.runtime.sendMessage(message).catch(() => {})
  }

  // get company name and position from linkedin job page
  const snapLinkedin = (): JobPostingMessage => {
  ...
  // get company name and position from glassdoor job page
  const snapGlassdoor = (): JobPostingMessage => {
    let message: JobPostingMessage = {
      message: 'jobSnap',
  ...
  switch (site) {
    case 'linkedinList':
      return sendMessage(snapLinkedin())
    case 'glassdoorList':
      return sendMessage(snapGlassdoor())
    default:
      return
  }
}
